...
def dockerBasePath = 'reagent-docker'

// Put all the Docker image building related jobs in a single folder
folder(dockerBasePath) {
  description 'Jobs concerning building Docker images for facebookresearch/ReAgent builds'
}

multiJob("reagent-docker-trigger") {
  label('simple')
...
Images.dockerImages.each {
  ...
  job("${dockerBasePath}/${buildEnvironment}") {
    ...
    scm {
      git {
        remote {
          github('facebookresearch/ReAgent')
          refspec([
...
image="${registry}/reagent/${JOB_BASE_NAME}"
...
cat > src/main/groovy/ossci/reagent/DockerVersion.groovy <<EOL
...
EOL
git add src/main/groovy/ossci/reagent/DockerVersion.groovy

git commit -m "Update reagent DockerVersion"
'''
  }
  publishers {
    git {
