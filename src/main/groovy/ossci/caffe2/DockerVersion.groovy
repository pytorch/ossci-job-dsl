// This file is manually managed
package ossci.caffe2
class DockerVersion {
  static final String version = "281";  // WARNING: if you change this to a new version number, you **MUST** also add that version number to the allDeployedVersions list below
  static final String allDeployedVersions = "281,276,273,266,253,248,238,230,213";  // NOTE: this is a comma-separated list. E.g. "262,220,219"
}
